...
| `data-embed-mode` | No | `inline` renders at the script position; `modal` renders a floating button that opens the form |
...
`embed=1` renders only the form (without the full hosted page layout), and the resize script keeps iframe height matched to form content.
