  ...
  _templateElements.forEach((otherEl) => {
    if (otherEl.type === "table" && otherEl.id !== el.id) {
      ...
      if (heightDiff > 0) {
        ...
        if (el.y >= originalTableBottom) adjustedY += heightDiff;
      }
    }
  });
  return adjustedY;
...
function renderTextElement(
  ...
  const typography = el.typography ?? { fontFamily: "Inter", fontSize: 12, fontWeight: "normal", lineHeight: 1.2, letterSpacing: 0, color: "#111827", align: "left" as const, uppercase: false, lowercase: false };
  return (
    <div key={el.id} style={style}>
      <div
        style={{
          ...
          textTransform: typography.uppercase ? "uppercase" : typography.lowercase ? "lowercase" : undefined,
          whiteSpace: "pre-wrap",
          width: "100%",
          height: "100%",
          overflow: "hidden",
...
function renderImageElement(
  ...
  if (el.binding) {
    ...
    if (boundValue) imageSrc = boundValue;
  }
  return (
    <div key={el.id} style={style}>
      {imageSrc ? (
        <img
          src={imageSrc}
          alt={el.alt || ""}
          style={{ width: "100%", height: "100%", objectFit: el.objectFit, display: "block", maxWidth: "100%", maxHeight: "100%" }}
        />
      ) : (
        <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", background: "#f3f4f6", color: "#6b7280", fontSize: 12, overflow: "hidden", boxSizing: "border-box" }}>
          No Image
        </div>
      )}
    </div>
...
function renderBoxElement(el: Extract<TemplateElement, { type: "box" }>, style: ElementStyle): React.ReactNode {
  return (
    <div
      key={el.id}
      style={{
...
function renderLineElement(el: Extract<TemplateElement, { type: "line" }>, style: ElementStyle): React.ReactNode {
  return (
    <div key={el.id} style={style}>
      <div style={{ borderTop: `${el.strokeWidth}px solid ${el.stroke}`, position: "absolute", left: 0, right: 0, top: "50%" }} />
    </div>
  );
}
...
function renderCurrencyElement(
  ...
  if (boundValue != null) {
    ...
    if (Number.isFinite(num)) {
      ...
      displayValue = new Intl.NumberFormat(undefined, { style: "currency", currency }).format(num);
    } else {
  ...
  return (
    <div key={el.id} style={style}>
      ...
      >
        <span style={{ fontSize: 10, color: "#6b7280", fontWeight: 500 }}>{el.currency || "USD"}</span>
        <span style={{ flex: 1 }}>{displayValue || el.placeholder || "0.00"}</span>
      </div>
    </div>
  );
}
...
function renderTableElement(
  ...
  const tableStyle = { ...style, height: totalTableHeight, overflow: "visible" as const };
    ...
    <div key={el.id} style={tableStyle}>
      <div style={{ width: "100%", height: "100%", fontSize: 10, color: "#374151", overflow: "visible", display: "flex", flexDirection: "column" }}>
        <div
          style={{
            display: "grid",
            gridTemplateColumns: el.columns.length > 0 ? el.columns.map((c) => `${c.width}px`).join(" ") : "1fr",
            ...
            return (
              <div
                ...
                style={{
                  display: "grid",
                  gridTemplateColumns: el.columns.length > 0 ? el.columns.map((c) => `${c.width}px`).join(" ") : "1fr",
                  borderBottom: el.stripe && actualIdx % 2 === 1 ? "1px solid #f3f4f6" : "1px solid #e5e7eb",
              ...
              >
                {el.columns.map((c) => {
                  ...
                  if (c.type === "currency") {
                    if (raw != null) {
                      ...
                      if (Number.isFinite(num)) {
                        ...
                        text = new Intl.NumberFormat(undefined, { style: "currency", currency }).format(num);
                      } else {
                  ...
                  return (
                    <div
                      key={c.id}
                      style={{ display: "flex", alignItems: "center", justifyContent: justify, padding: "4px", wordBreak: "break-word", overflowWrap: "break-word", minHeight: "20px" }}
                    >
                      {text}
                    </div>
                  );
            ...
            >
              {el.columns.map((c) => {
                ...
                if (c.showTotal && (c.type === "number" || c.type === "currency")) {
                  try {
                    const columnBinding = c.binding || c.id;
                    const sum = allItems.reduce((s, row) => {
                    ...
                    if (c.totalStyle) {
                      if (c.totalStyle.backgroundColor) cellStyle.backgroundColor = c.totalStyle.backgroundColor;
                      if (c.totalStyle.color) cellStyle.color = c.totalStyle.color;
                      if (c.totalStyle.fontWeight) cellStyle.fontWeight = c.totalStyle.fontWeight;
                      if (c.totalStyle.fontSize) cellStyle.fontSize = c.totalStyle.fontSize;
                      if (c.totalStyle.borderTop) cellStyle.borderTop = c.totalStyle.borderTop;
                    } else {
...
export function renderTemplateElement(el: TemplateElement, renderContext: RenderContext): React.ReactNode {
