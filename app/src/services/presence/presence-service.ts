...
export const presenceService: PresenceService = {
	async joinTemplate(
		...
		console.log(
			"PresenceService: Attempting to join template",
			templateId,
			"for user",
			user.uid
		);
		...
		const presenceRef = ref(
			firebase.database,
			`${PRESENCE_PATH}/${templateId}/${user.uid}`
		);
		...
		try {
			...
			onValue(connectedRef, (snapshot) => {
				if (snapshot.val() === true) {
					...
					console.log(
						"PresenceService: User connected and presence set for",
						user.uid
					);
				} else {
			...
			console.log(
				"PresenceService: Successfully set up presence for",
				user.uid
			);
		} catch (error) {
	...
	async leaveTemplate(templateId: string, user: AuthUser) {
		...
		try {
			...
			console.log(
				"PresenceService: Successfully left template presence for",
				user.uid
			);
		} catch (error) {
			console.error(
				"PresenceService: Failed to remove presence data:",
				error
			);
			throw error;
		}
	...
	async updateSelection(
		...
		try {
			...
			console.error(
				"PresenceService: Failed to update selection:",
				error
			);
			throw error;
		}
	...
	subscribeToPresence(
		...
		const handlePresenceUpdate = (snapshot: {
			...
			if (!presenceData) {
				console.log(
					"PresenceService: No presence data, sending empty array"
				);
				callback([]);
				return;
			}
			...
			const users: UserPresence[] = allUsers
				.filter((user: UserPresence) => {
					...
					if (!isValid) {
						console.log(
							"PresenceService: Filtering out invalid user",
							user
						);
					}
					if (!isRecent) {
						console.log(
							"PresenceService: Filtering out stale user",
							user.uid,
							"last seen:",
				...
				.map((user: UserPresence) => ({
					...
					isOnline:
						user.isActive && Date.now() - user.lastSeen < 30000, // Online if active and seen in last 30 seconds
		...
		console.log(
			"PresenceService: Setting up presence subscription for template",
			templateId
		);
