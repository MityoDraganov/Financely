...
export function getTablePercentageWidthSum(
	columns: TableColumnLike[],
	excludedColumnId?: string
): number {
	return roundTo(
		columns.reduce((sum, column) => {
			if (excludedColumnId && column.id === excludedColumnId) return sum;
			const parsed = parseWidthToken(column.width);
			if (!parsed || parsed.unit !== "%") return sum;
			return sum + parsed.value;
		}, 0)
	);
}

export function clampPercentageColumnWidth(
	nextValue: number,
	columns: TableColumnLike[],
	columnId: string
): number {
	const safeValue = Math.max(0, Number.isFinite(nextValue) ? nextValue : 0);
	const usedByOthers = getTablePercentageWidthSum(columns, columnId);
	const maxAllowed = Math.max(0, 100 - usedByOthers);
	return roundTo(Math.min(safeValue, maxAllowed));
}
